...
export const sendMessage = async (req, res, next) => {
    try {
        ...
        console.log(`[MESSAGE PROBE] Sending message from: ${req.user?.name} (${senderId}), Content: "${content.substring(0, 20)}..."`);
        if (!senderId) {
        ...
        try {
            const io = getIO();
            console.log(`📤 [DEFENSIVE] Broadcasting message from ${responseData.senderId.name} to room: ${teamId.toString()}`);
